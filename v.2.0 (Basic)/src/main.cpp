...
String smt1, smt2, smt3, smt4, jadiraw,jadiraw2;
String payload;

uint16_t id;

//Deklarasi
const char* SSID_AP ="AWLMS"; //Site A
...
uint8_t jaam, meenit, haari, bulaan;
...
// void cekfile5(){
//   myfile = SD.open("/quicksend.txt");
//   if(!myfile) {
//     Serial.println("File doesn't exist");
//     Serial.println("Creating file...");
//     myfile = SD.open("/quicksend.txt",FILE_WRITE);
//   }
//   else {
//     Serial.println("File already exists");  
//   }
...
//belum diubah
void logSDCard() {
  dataMessage = "&D=" + tanggal2 + "&T=" + waktu + "&C0=0&C1=0&C2=0&C3=" + String(nilaiValue) +
                "&C13=" + String(voltage) + "&C14=" + 
                String(power) + "&R=3" + '\n';
...
void sendToServer() {
  String requestData = "&C0=" + String(voltage) + "&C1=0&C2=" 
                        + String(power) + "&C3=0.00&C4=0.00&C5=0&C6=0&C7=0.00&C8=0.00&C9=0.00&C10=0.00&C11=0.00&C12=" 
                        + String(nilaiAkhir) + "&C13=" + String(nilaiValue);
...
String print() {
  String nilaisave = "Ketinggian Air = " + String(nilaiValue) + ", Voltage = "+ String(voltage) + ", Power = " + String(power) +'\n';
  return nilaisave;
}
...
void setup() {
  ...
  // Max44009 myLux(alamat_gy);
  // myLux.setContinuousMode();
  // myLux.getConfiguration();
...
void loop() {
  ...
  if(!maintananceState){
    ...
    if((bagi != 0) && (detik2 != 0)){
      ...
      // float nilai_maxbotix[50];
      // float maxbotix[50];

      // for(int abc=0;abc<50;abc++){
      //   nilaiAwal = analogRead(34);
      //   nilai_maxbotix[abc] = (nilaiAwal * 0.516) + 63.15;
      //   delay(100);
      // }

      // for(int abd=0;abd<50-1;abd++){
      //   for(int abe=0;abe<50-abd-1;abe++){
      //     if(nilai_maxbotix[abe] > nilai_maxbotix[abe+1]){
      //       float temp = nilai_maxbotix[abe];
      //       nilai_maxbotix[abe] = nilai_maxbotix[abe+1];
      //       nilai_maxbotix[abe+1] = temp;
      //     }
      //   }
      // }

      // for(int j=0;j<50;j++){
      //   maxbotix[j] = (nilai_maxbotix[j] + nilai_maxbotix[j+1]) / 2.00;
      ...
      for(int il2=0;il2<50;il2++){
        ...
        nilaiAwal = analogRead(34);
        nilai_tinggi[il2] = (nilaiAwal * 0.516) + 63.15;
        nilai_tinggi[il2] = (nilai_tinggi[il2] * a_KetinggianAir) + c_KetinggianAir;
        if(il2 < 49){
          jadiraw = String(nilai_tinggi[il2]) + ',';
          appendFile(SD, "/raw_tinggi.csv", jadiraw.c_str());
          String nah = "Ke - " + String(il2+1);
          Serial.println(nah);
        } else {
          jadiraw2 = String(nilai_tinggi[il2]) + "\n";
          appendFile(SD, "/raw_tinggi.csv", jadiraw2.c_str());
          Serial.println("ke-50");
        }
        delay(300);
      }
      ...
      for(int il3=0;il3<50;il3++){
        nilaiAkhir += nilai_tinggi[il3];
        String nilai = String(nilai_tinggi[il3]) + ',';
        Serial.print(nilai);
      }
      
      nilaiAkhir = nilaiAkhir / 50.00;
      Serial.println(nilaiAkhir);
      // nilaiAwal = nilai_awal/50.00;
      // nilaiConv = (nilaiAwal * 0.516) + 63.15;
      // nilaiAkhir = (nilaiConv * a_KetinggianAir) + c_KetinggianAir;
      ...
      cekfile4();
      // uint16_t id2 = id + 1;
      // SD.remove("/id_data.txt");

      // String id_da = String(id2);
      // writeFile(SD, "/id_data.txt", id_da.c_str());
      delay(500);
      
      String jadicsv = String(tanggal2) + ',' + String(waktu) + ',' + String(nilaiValue) +"\n";
      appendFile(SD, "/dataraw.csv", jadicsv.c_str());
      if (!modem.gprsConnect(apn, gprsUser, gprsPass)) {
        Serial.println(" fail to Connect to GPRS");
        cekfile();
        logSDCard();
    ...
    // delay(5000);
    // String quicksend = readFile(SD,"/quicksend.txt");
    // if(quicksend == "1"){
    //   // serverService.end();
    //   WiFi.mode(WIFI_STA);
    //   WiFi.disconnect();
    //   delay(100);
    //   Serial.print("Connecting to ");
    //   const char * ssid = "UGM-Hotspot-STP";
    //   const char * pass = "stpugm2019";
    //   Serial.println(ssid);
    ...
    //   HTTPClient http;
    //   WiFiClient wifi;
    //   String req = "http://api.smartagri.id/?key=apikeyriset&C0=11&C1=22&C3=33";
    //   Serial.println(req);
    ...
    //     String version = obj[String("valid")];
    ...
    //       String tes = "0";
    //       writeFile(SD,"/quicksend.txt",tes.c_str());
